package blog::Web::Plugin::Session;
use strict;
use warnings;
use utf8;
...
sub init {
    ...
    $c->add_trigger(
        BEFORE_DISPATCH => sub {
            my ( $c ) = @_;
            if ($c->req->method ne 'GET' && $c->req->method ne 'HEAD' && $ENV{"AMON2_TESTING"} ne 1) {
...
sub _session {
    ...
    if (!exists $self->{session}) {
        $self->{session} = HTTP::Session2::ClientStore->new(
            env => $self->req->env,
            secret => 'MyiAqWKmgPeUprFI4LIn64PvREK3rRvw',
        );
    }
    return $self->{session};
}
...
This module manages session for blog.
